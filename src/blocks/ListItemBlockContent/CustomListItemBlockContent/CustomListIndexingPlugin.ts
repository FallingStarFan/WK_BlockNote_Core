...
const PLUGIN_KEY = new PluginKey(`custom-list-indexing`);
export const CustomListIndexingPlugin = () => {
  return new Plugin({
    key: PLUGIN_KEY,
    appendTransaction: (_transactions, _oldState, newState) => {
      const tr = newState.tr;
      tr.setMeta("customListIndexing", true);
      ...
      newState.doc.descendants((node, pos) => {
        if (
          ...
          node.firstChild!.type.name === "customListItem"
        ) {
          ...
          if (prevBlock) {
            ...
            const isPrevBlockOrderedListItem =
              prevBlockInfo.blockNoteType === "customListItem";
          ...
          const isFirst =
            prevBlock?.firstChild?.type.name !== "customListItem";
