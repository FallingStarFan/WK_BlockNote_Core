...
export function getCustomContent(
  ...
  // Causing it to be recognized as a `checkCustom`.
  // We want to skip this, and just parse the list item's content as is.
  if (blockGroupNode.firstChild?.firstChild?.type.name === "checkCustom") {
  ...
  // So, we extract it, and insert its content into the `customNode`.
  // The remaining nodes in the `blockGroup` stay in-place.
  const customNode = schema.nodes[name].create(
    {},
    customsFirstChild.content,
  );

  // We have `blockGroup<customsFirstChild, ...blockContainer<any>[]>`
  // We want to extract out the rest of the nodes as `<...blockContainer<any>[]>`
  const remainingCustomChildren = blockGroupNode.content.cut(
    // +2 for the `blockGroup` node's start and end markers
    customsFirstChild.nodeSize + 2,
  );
  const hasRemainingCustomChildren = remainingCustomChildren.size > 0;

  if (hasRemainingCustomChildren) {
    ...
    const customsChildren = blockGroupNode.copy(remainingCustomChildren);

    // Return the `custom` node's content, then add the parsed children after to be lifted out by ProseMirror "fitting" algorithm.
    return customNode.content.addToEnd(customsChildren);
  }

  // Otherwise, just return the `custom` node's content.
  return customNode.content;
}
