...
export const IndentCustomIndexingPlugin = () => {
  return new Plugin({
    key: PLUGIN_KEY,
    appendTransaction: (_transactions, _oldState, newState) => {
      const tr = newState.tr;
      tr.setMeta("indentCustomIndexing", true);
      ...
      newState.doc.descendants((node, pos) => {
        if (
          ...
          node.firstChild!.type.name === "indentCustomItem"
        ) {
          ...
          if (prevBlock) {
            ...
            const isPrevBlockOrderedListItem =
              prevBlockInfo.blockNoteType === "indentCustomItem";
          ...
          const isFirst =
            prevBlock?.firstChild?.type.name !== "indentCustomItem";
